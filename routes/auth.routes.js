...
router.post(
  '/register',
  [
    check('email', 'Не корретный email').isEmail(),
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Что-то пошло не так, попробуте снова' })
    }
  })
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!isMatch) {
        return res.status(400).json({ message: 'Не верный пароль, поробуйте снова' })
      }
